...
func (mgr *ImagePackMgr) ListAll(c *gin.Context) {
	...
	var err error
	if token.ClusterRole == model.RoleUser {
		imagepacks, err = imagepackQuery.WithContext(c).Where(imagepackQuery.ProjectID.Eq(token.ProjectID)).Find()
	} else if token.ClusterRole == model.RoleAdmin {
		listType := c.DefaultQuery("type", "0")
		if listType == "0" {
			imagepacks, err = imagepackQuery.WithContext(c).Where(
				imagepackQuery.ProjectID.Neq(PublicProjectID),
			).Where(
				imagepackQuery.Status.Neq(ImagePackDeleted),
			).Find()
		} else if listType == "1" {
			imagepacks, err = imagepackQuery.WithContext(c).Where(
				imagepackQuery.ProjectID.Eq(PublicProjectID),
			).Where(
				imagepackQuery.Status.Neq(ImagePackDeleted),
			).Find()
		} else {
